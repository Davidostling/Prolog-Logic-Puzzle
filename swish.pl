...
fact4(Ls) :-
	member(p(WX, _, _, _, first), Ls), not(WX==w2).
fact5(Ls) :-
	member(p(_, _, cleaning, _, basement), Ls).
fact6(Ls) :-
	member(p(WX, _, _, full, ground), Ls), member(p(WY, _, _, full, ground), Ls), not(WX==WY).
fact7(Ls) :-
	member(p(_, 190, beverage, _, _), Ls).
fact8(Ls) :-
	member(p(WX, _, _, part, _), Ls), member(p(WY, _, _, part, _), Ls), not(WX==WY), not(WX==w3), not(WY==w3).
fact9(Ls) :-
	member(p(_, 170, _, _, first), Ls).
fact10(Ls) :-
	member(p(_, _, _, _, first), Ls), member(p(_, _, _, _, basement), Ls). 
